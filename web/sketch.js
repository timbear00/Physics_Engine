class Mover {
  constructor(_x, _y, _mass, _radius){
  ...
  update() {
    ...
    this.position.add( p5.Vector.mult(this.velocity, 6) );
...
let mass;
let radius;
let gravitational_acceleration = 10;
let time;
let mode;
let obj_cliked = false;
let angle;
let move=0;
...
function setup() {
  ...
  m = new Mover(width/4, 50, mass, radius);
...
function draw() {
  ...
  if ( mouseIsPressed==true && mode==2 && obj_cliked==true ) {
    cal_angle();
  }

  let gravity = createVector(0, mass * gravitational_acceleration);
...
function keyPressed() {
  ...
  // 각각의 mode에서의 행동
  if (mode==1) {
    let user_force = createVector(0,0);
    if( keyCode==RIGHT_ARROW ) user_force.x += 30;
    if (keyCode==LEFT_ARROW) user_force.x -= 30;
    if (keyCode==UP_ARROW) user_force.y -= 100 * mass;
    if (keyCode==DOWN_ARROW) user_force.y += 30;
...
function mousePressed() {
  if ( mode==2) {
    let distance = dist(mouseX, mouseY, radius/2 + 50, height-(radius/2));    // Object와 마우스 사이의 거리 측정
...
function mouseReleased() {
  if (mode==2 && obj_cliked==true) {
    let force = createVector( 350*cos(radians(angle)), 350*sin(radians(angle)) );
...
function defaultText() {
  ...
  textAlign(CENTER, CENTER);
  textSize(45);
  text("Mode : "+mode, 150, 60);

  if (mode==1) {
    textAlign(CENTER, BOTTOM);
    textSize(25);
    text("Press 1,2 to change mode / Press Space to stop", width/2, height-20);
  }
  else if (mode==2) {
    textAlign(CENTER, BOTTOM);
    textSize(25);
...
function cal_angle() {
  let start_x = radius/2 + 50;
  let start_y = height-(radius/2);
  let distance = dist(mouseX, mouseY, start_x, start_y);    // Object와 마우스 사이의 거리 측정
  noFill();
  stroke(0);
  strokeWeight(3);
